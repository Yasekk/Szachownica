...
def end_game(chess_board,screen,settings,message,game_end):
	"""Uzunięcie planszy i pokazanie komunikatu końca gry"""
	...
	#pole (jeżeli jest to dozwolony ruch)	
		...
		for board_part in chess_board:
			...
			board_part.name in settings.avialable_moves):
				...
				screen,settings,king_attack_path)
...
settings,king_attack_path):
	...
	if settings.current_move=="white":
		...
		for board_part in chess_board:
			if board_part.occupied:
				if (board_part.occupied.type=="black" and 
				board_part.occupied.piece_name!="king"):
					for move in board_part.occupied.moves:
						if move in king_attack_path:
							skip_checkmate=True
				...
				#następuje szach mat
				elif (board_part.occupied.type=="white" and 
				board_part.occupied.piece_name=="king"):
